...
export type SeedResult = {
  users: string[];
  notes: { title: string; roles: Record<string, string> }[];
};

/** Тестовые пользователи и конспекты (перезаписывает notes/comments/blocks, users — upsert). */
export async function runSeedDatabase(): Promise<SeedResult> {
